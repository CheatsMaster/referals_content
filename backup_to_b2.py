...
class B2Backup:
    ...
    def init_b2_client(self):
        ...
        if not self.b2_key_id or not self.b2_app_key:
            logger.error("❌ B2_KEY_ID или B2_APP_KEY не установлены!")
            return False
...
def schedule_backups():
    ...
    # Создаем бэкап сразу при запуске
    logger.info("🚀 Выполняю начальный бэкап...")
    backup_job()
    
    # Настраиваем расписание
    schedule.every().hour.do(backup_job)  # каждый час
    schedule.every().day.at("03:00").do(lambda: B2Backup().cleanup_old_backups(keep_hours=168))  # раз в неделю
...
if __name__ == "__main__":
    ...
    required_vars = ['B2_KEY_ID', 'B2_APP_KEY']
    ...
    if missing_vars:
        ...
        logger.error("   - B2_APP_KEY: Application Key из Backblaze B2")
        logger.error("   - B2_BUCKET: telegram-bot-backups (опционально)")
        sys.exit(1)
